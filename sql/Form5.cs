    ...
    public partial class Form5 : Form
    {
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0)
            {
                ...
                textBox2.Text = satir.Cells["OrderID"].Value.ToString();
                textBox4.Text = satir.Cells["EmployeeID"].Value.ToString();
                textBox1.Text = satir.Cells["CustomerID"].Value.ToString();
        ...
        private void dataGridView1_CellValueChanged(object sender, DataGridViewCellEventArgs e)//SENDER: OLAYI TETİKLEYEN NESNE E: İSE OLAY PARAMETRELERİ
        {
            if (e.RowIndex >= 0) //GERÇEK SATIR MI KONTROLÜ YAPAR
            {
                try//HATA ÖNLEME
                {
                    ...
                    int id = Convert.ToInt32(satir.Cells["OrderID"].Value);
                    string yenimüsteri = satir.Cells["EmployeeID"].Value.ToString();
                    string siparisialancalısan = satir.Cells["CustomerID"].Value.ToString();


                    baglanti.Open();
                    SqlCommand komut = new SqlCommand("UPDATE Orders SET  EmployeeID=@p1 , CustomerID=@p2  WHERE OrderID = @p3", baglanti);
                    komut.Parameters.AddWithValue("@p1", yenimüsteri);
                    komut.Parameters.AddWithValue("@p2", siparisialancalısan);
        ...
        private void Siparisekleme()
        {
            baglanti.Open();
            SqlCommand komut = new SqlCommand("INSERT INTO Orders (EmployeeID,CustomerID) VALUES (@p1,@p2)", baglanti);
        ...
        private void Siparissilme()
        {
            baglanti.Open();

            // Önce Order Details tablosundaki ilişkili kayıtları sil
            SqlCommand komut1 = new SqlCommand("DELETE FROM [Order Details] WHERE OrderID IN (SELECT OrderID FROM Orders WHERE EmployeeID=@p1 AND CustomerID=@p2)", baglanti);
            komut1.Parameters.AddWithValue("@p1", textBox4.Text);
            komut1.Parameters.AddWithValue("@p2", textBox1.Text);
            komut1.ExecuteNonQuery();

            // Sonra Orders tablosundaki siparişi sil
            SqlCommand komut2 = new SqlCommand("DELETE FROM Orders WHERE EmployeeID=@p1 AND CustomerID=@p2", baglanti);
        ...
        private void SiparisAra()
        {
            baglanti.Open();
            SqlDataAdapter da = new SqlDataAdapter("SELECT * FROM Orders WHERE EmloyeeID LIKE @p1", baglanti);
